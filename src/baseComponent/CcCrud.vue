...
import CrudUtils from '@utils/CrudUtils.jsx'
...
export default class CcCrud extends Vue {
  ...
  // 所以折中的办法是后端传方法名，前端通过方法名找到对应的方法。这种情况需要前后端配合才能完成相应的业务
   ...
   get rowKey () {
     return this.data.table.props && this.data.table.props.rowKey ? this.data.table.props.rowKey : 'id'
   }
   get multi () {
     return !!this.data.table.columns.find((c) => c.type === 'selection')
   }
        ...
        if (this.value) {
          if (this.multi) {
            this.selectedRows.forEach((row, index) => {
              const newRow = data.list.find((row2) => row[this.rowKey] === row2[this.rowKey])
              if (newRow) {
            ...
            if (this.currentRow) {
              const newRow = data.list.find((row) => row[this.rowKey] === this.currentRow[this.rowKey])
              if (newRow) {
